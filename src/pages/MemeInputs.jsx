 ...
 const handleImageToMeme = async () => {
   ...
   try {
     ...
     if (activeButton == "prompt") {
       const response = await axios.post(
         "https://memeverse.onrender.com/text-to-video-meme",
         formData,
         {
           headers: {
       ...
       const response = await axios.post(
         "https://memeverse.onrender.com/upload-video-to-meme",
         formData,
         {
           headers: {
  ...
  const handleCameraClick = () => {
    ...
    const handlePromptToMeme = async () => {
      setLoader(true);
      let _link;
      console.log(activeButton1,"activebutton")
      try {
        const formData = { prompt: context };
        console.log(context);
        // formData.append("image", image);
        // formData.append("prompt", context);
        if (activeButton1 == "template") {
          const response = await axios.post(
            "https://memish.onrender.com/text-to-template-meme",
            formData,
            {
              headers: {
                "Content-Type": "application/json",
          ...
          const response = await axios.post(
            "https://memish.onrender.com/text-to-anime-image-gen-meme",
            formData,
            {
              headers: {
                "Content-Type": "application/json",
