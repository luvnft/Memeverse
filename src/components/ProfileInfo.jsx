...
function ProfileStats({ tokens, generations }) {
  const stats = [
        ...
        <div className="flex gap-5 max-md:flex-col">
          ...
          <div className="flex flex-col ml-5 w-[61%] max-md:ml-0 max-md:w-full">
            <header className="flex flex-col items-start mt-3.5 w-full text-xl text-white max-md:mt-10">
              <h1 className="text-2xl font-bold">{profile && profile['name']}</h1>
              <p className="mt-1 text-neutral-500">@{profile && profile['username']}</p>
              <p className="mt-1 text-neutral-500">#{profile && profile['interests']}</p>
              <ProfileStats generations={profile && profile['generations']} tokens={profile && profile['tokens']} />
            </header>
          </div>
        </div>
      </div>
